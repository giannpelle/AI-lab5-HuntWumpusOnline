...
PIT_PROBABILITY = 0.2
MIN_SAFE_PROBABILITY_TRESHOLD = 0.8
IS_SQUARE_SIZE = True
...
class KnowledgeBase(object):
    ...
    def update_with(self, *, previous_agent_action, current_percept):
        ...
        """
        logger.debug("")
        logger.debug(f"Before {previous_agent_action} state: {self}")
        logger.debug("")

        if previous_agent_action is not None:
            if previous_agent_action == Hunter.Actions.MOVE and not current_percept.bump:
                logger.debug("UPDATED")
                logger.debug(f"agent_location from: {self.agent_location} to: {self.agent_location + self.agent_orientation}") 
                self.agent_location = self.agent_location + self.agent_orientation
            
            elif previous_agent_action == Hunter.Actions.RIGHT:
                logger.debug("UPDATED")
                logger.debug(f"agent_orientation from: {self.agent_orientation} ")
                logger.debug(f"to: {self.agent_orientation.get_perpendicular_vector_clockwise()}")
                ...
                logger.debug("UPDATED" if self.is_arrow_available != False else "NOT UPDATED")
                logger.debug(f"is_arrow_available from: {self.is_arrow_available} to: {False}")
        ...
        if self.agent_location in self.visited_locations:
            if current_percept.bump:
                if IS_SQUARE_SIZE:
                    logger.debug(f"discovered the size of the squared world")
                    logger.debug(f"has_agent_bumpep from: {self.has_agent_bumped_north} to: {True}")
                    self.has_agent_bumped_north = True
                    logger.debug(f"has_agent_bumped_east from: {self.has_agent_bumped_east} to: {True}")
                    self.has_agent_bumped_east = True
                else:
                    if self.agent_orientation == SmartVector(0, 1):
                        logger.debug("UPDATED")
                        logger.debug(f"has_agent_bumped_north from: {self.has_agent_bumped_north} to: {True}")
                        self.has_agent_bumped_north = True
                ...
                logger.debug("UPDATED" if self.fringe_locations != self.filter_locations_from_unknown_world(from_locations=self.fringe_locations) else "NOT UPDATED")
                logger.debug(f"fringe_locations from: {get_pretty_log_representation_of(locations=self.fringe_locations)}")
                logger.debug(f" to: {get_pretty_log_representation_of(locations=self.filter_locations_from_unknown_world(from_locations=self.fringe_locations))}")
                self.fringe_locations = self.filter_locations_from_unknown_world(from_locations=self.fringe_locations)

                logger.debug("UPDATED" if self.no_pit_locations != self.filter_locations_from_unknown_world(from_locations=self.no_pit_locations) else "NOT UPDATED")
                logger.debug(f"no_pit_locations from: {get_pretty_log_representation_of(locations=self.no_pit_locations)} ")
                logger.debug(f"to: {get_pretty_log_representation_of(locations=self.filter_locations_from_unknown_world(from_locations=self.no_pit_locations))}")
                self.no_pit_locations = self.filter_locations_from_unknown_world(from_locations=self.no_pit_locations)
                
                self.update_wumpus()

            logger.debug(f"After {previous_agent_action} and percept {current_percept} -> state: \n{self}")
            logger.debug("")
        ...
        if IS_SQUARE_SIZE and (self.agent_location.x not in range(0, self.world_size_width) \
            ...
            logger.debug(f"increased square size:")
            logger.debug(f"world size: width from: {self.world_size_width} to: {self.world_size_width + 1}")
            self.world_size_width += 1
            logger.debug(f"world size: height from: {self.world_size_height} to: {self.world_size_height + 1}")
            self.world_size_height += 1
        else:
        ...
        logger.debug("UPDATED" if self.agent_location not in self.no_pit_locations else "NOT UPDATES")
        logger.debug(f"no_pit_locations from: {get_pretty_log_representation_of(locations=self.no_pit_locations)} ")
        logger.debug(f"to: {get_pretty_log_representation_of(locations=self.no_pit_locations.union(set([self.agent_location])))}")
        self.no_pit_locations.add(self.agent_location)

        logger.debug("UPDATED" if self.agent_location not in self.no_wumpus_locations else "NOT UPDATED")
        logger.debug(f"no_wumpus_locations from: {get_pretty_log_representation_of(locations=self.no_wumpus_locations)} ")
        logger.debug(f"to: {get_pretty_log_representation_of(locations=self.no_wumpus_locations.union(set([self.agent_location])))}")
        self.no_wumpus_locations.add(self.agent_location)

        logger.debug("UPDATED" if self.agent_location not in self.visited_locations else "NOT UPDATED")
        logger.debug(f"visited_locations from: {get_pretty_log_representation_of(locations=self.visited_locations)} ")
        logger.debug(f"to: {get_pretty_log_representation_of(locations=self.visited_locations.union(set([self.agent_location])))}")
        ...
        logger.debug("UPDATED")
        logger.debug(f"fringe_locations from: {get_pretty_log_representation_of(locations=self.fringe_locations)}")
        logger.debug(f"to: {get_pretty_log_representation_of(locations=self.fringe_locations.difference(set([self.agent_location])))}" if self.fringe_locations != self.fringe_locations.union(inner_not_visited_neighbour_locations) else "")
        logger.debug(f"to: {get_pretty_log_representation_of(locations=self.fringe_locations.union(inner_not_visited_neighbour_locations).difference(set([self.agent_location])))}" if not(self.fringe_locations != self.fringe_locations.union(inner_not_visited_neighbour_locations)) else "")
        ...
        if current_percept.breeze:
            logger.debug("UPDATED" if self.agent_location not in self.breeze_locations else "NOT UPDATED")
            logger.debug(f"breeze_locations from: {get_pretty_log_representation_of(locations=self.breeze_locations)} ")
            logger.debug(f"to: {get_pretty_log_representation_of(locations=self.breeze_locations.union(set([self.agent_location])))}")
            ...
            logger.debug("UPDATED" if self.agent_location not in self.no_breeze_locations else "NOT UPDATED")
            logger.debug(f"no_breeze_locations from: {get_pretty_log_representation_of(locations=self.no_breeze_locations)} ")
            logger.debug(f"to: {get_pretty_log_representation_of(locations=self.no_breeze_locations.union(set([self.agent_location])))}")
            ...
            logger.debug("UPDATED" if self.no_pit_locations != self.no_pit_locations.union(inner_not_visited_neighbour_locations) else "NOT UPDATED")
            logger.debug(f"no_pit_locations from: {get_pretty_log_representation_of(locations=self.no_pit_locations)} ")
            logger.debug(f"to: {get_pretty_log_representation_of(locations=self.no_pit_locations.union(inner_not_visited_neighbour_locations))}")
    ...
    def update_wumpus(self, *, with_possible_wumpus_locations=set(), with_no_wumpus_locations=set(), scream=False):
        ...
        logger.debug("UPDATED" if self.no_wumpus_locations != self.no_wumpus_locations.union(no_wumpus_locations) else "NOT UPDATED")
        logger.debug(f"no_wumpus_locations from: {get_pretty_log_representation_of(locations=self.no_wumpus_locations)} ")
        logger.debug(f"to: {get_pretty_log_representation_of(locations=self.no_wumpus_locations.union(no_wumpus_locations))}")
        self.no_wumpus_locations = self.no_wumpus_locations.union(no_wumpus_locations)
        
        logger.debug("UPDATED" if self.no_wumpus_locations != self.filter_locations_from_unknown_world(from_locations=self.no_wumpus_locations) else "NOT UPDATED")
        logger.debug(f"no_wumpus_locations from: {get_pretty_log_representation_of(locations=self.no_wumpus_locations)} ")
        logger.debug(f"to: {get_pretty_log_representation_of(locations=self.filter_locations_from_unknown_world(from_locations=self.no_wumpus_locations))}")
        self.no_wumpus_locations = self.filter_locations_from_unknown_world(from_locations=self.no_wumpus_locations)
        
        logger.debug("UPDATED" if self.possible_wumpus_locations != self.filter_locations_from_unknown_world(from_locations=self.possible_wumpus_locations) else "NOT UPDATED")
        logger.debug(f"possible_wumpus_locations from: {get_pretty_log_representation_of(locations=self.possible_wumpus_locations)} ")
        logger.debug(f"to: {get_pretty_log_representation_of(locations=self.filter_locations_from_unknown_world(from_locations=self.possible_wumpus_locations))}")
        ...
        if scream:
            logger.debug(f"no_wumpus_locations from: {self.no_wumpus_locations} ")
            logger.debug(f"to: {self.no_wumpus_locations.union(self.possible_wumpus_locations)}")
            self.no_wumpus_locations = self.no_wumpus_locations.union(self.possible_wumpus_locations)

            logger.debug(f"is_wumpus_alive from: {self.is_wumpus_alive} to: {False}")
            self.is_wumpus_alive = False

            logger.debug(f"known_wumpus_location from: {self.known_wumpus_location} to: {None}")
            self.known_wumpus_location = None

            logger.debug(f"possible_wumpus_locations from: {get_pretty_log_representation_of(locations=self.possible_wumpus_locations)} ")
            logger.debug(f"to: {get_pretty_log_representation_of(locations=[])}")
            self.possible_wumpus_locations = set()
            return
        ...
        if self.known_wumpus_location is None and self.is_wumpus_alive:
            if possible_wumpus_locations:
                if not self.possible_wumpus_locations:
                    logger.debug(f"possible_wumpus_locations from: {get_pretty_log_representation_of(locations=self.possible_wumpus_locations)} ")
                    logger.debug(f"to: {get_pretty_log_representation_of(locations=possible_wumpus_locations)}")
                    self.possible_wumpus_locations = possible_wumpus_locations
                else:
                    logger.debug("UPDATED" if self.possible_wumpus_locations != self.possible_wumpus_locations.intersection(possible_wumpus_locations) else "NOT UPDATED")
                    logger.debug(f"possible_wumpus_locations from: {get_pretty_log_representation_of(locations=self.possible_wumpus_locations)} ")
                    logger.debug(f"to: {get_pretty_log_representation_of(locations=self.possible_wumpus_locations.intersection(with_possible_wumpus_locations))}")
                    self.possible_wumpus_locations = self.possible_wumpus_locations.intersection(possible_wumpus_locations)

            logger.debug("UPDATED" if self.possible_wumpus_locations != self.possible_wumpus_locations.difference(self.no_wumpus_locations) else "NOT UPDATED")
            logger.debug(f"possible_wumpus_locations from: {get_pretty_log_representation_of(locations=self.possible_wumpus_locations)} ")
            logger.debug(f"to: {get_pretty_log_representation_of(locations=self.possible_wumpus_locations.difference(self.no_wumpus_locations))}")
            ...
            if len(self.possible_wumpus_locations) == 1:
                logger.debug(f"known_wumpus_location from: {self.known_wumpus_location} to: {list(self.possible_wumpus_locations)[0]}")
        ...
        if self.known_wumpus_location and self.is_wumpus_alive:
            logger.debug("Agent perceiving stench but already knows where the wumpus is, therefore all neighbours except the wumpus locations are no_wumpus")
            self.no_wumpus_locations = self.no_wumpus_locations.union(possible_wumpus_locations.difference(set([self.known_wumpus_location])))
    ...
    def get_bayesian_model(self):
        ...
        for breeze_cpd in breeze_cpds.values():
            logger.debug("breeze bayesian network disjoint probability table:")
            logger.debug(f"{breeze_cpd}")
    ...
    def get_evidence_dict(self):
        ...
        logger.debug(f"evidence_dict for current KnowledgeBase state: \n{evidence_dict}")
    ...
    def check_pit_probability_of(self, *, location, bayesian_model, evidence_dict):
        ...
        logger.debug("")
        logger.info(f"pit probability of {location}: {exp_infer.query([f'P{location.x}{location.y}'], evidence=evidence_dict, show_progress=False).values[0]}")
        logger.debug("")
    ...
    def check_safe_probability_of(self, *, location, bayesian_model, evidence_dict):
        ...
        logger.debug("")
        logger.info(f"safe  probability of {location}: {(1 - pit_probability) * (1 - wumpus_probability)}")
        logger.debug("")
...
class HuntWumpusHybridAgent(object):
    ...
    def get_next_action_from(self, *, percept):
        ...
        """
        logger.info("-" * 15 + "Updating KnowledgeBase info" + "-" * 15)
        logger.info("")
        self.kb.update_with(previous_agent_action=self.previous_agent_action, current_percept=percept)

        logger.info(f"Current agent_location: {self.kb.agent_location}, orientation: {self.kb.agent_orientation}")

        if percept.glitter:
            if self.plan_actions:
                logger.critical("Found Gold executing other plans (inadmissible heuristic probably)")
                self.plan_actions = []

            logger.info("")
            logger.warning("*" * 7 + " " * 2 + "Reached the gold, calculating way back to exit")
            ...
            logger.info(f"Heading to location {get_final_location(from_location=self.kb.agent_location, with_orientation=self.kb.agent_orientation, with_plan_actions=self.plan_actions)[0]} ")
            logger.info(f"with plan cost {get_final_location(from_location=self.kb.agent_location, with_orientation=self.kb.agent_orientation, with_plan_actions=self.plan_actions)[1]}")
            logger.info(f"Way back plan: {self.plan_actions}\n")
        
        if not self.plan_actions:
            logger.info("")
            logger.warning("*" * 7 + " " * 2 + "Calculating plan to visit the closest unvisited safe location")
            ...
            if self.kb.known_wumpus_location is not None and self.kb.known_wumpus_location in self.kb.no_pit_locations:
                logger.info(f"Unvisited safe locations: {self.kb.get_safe_unvisited_locations()} and known wumpus location {self.kb.known_wumpus_location}")
                ...
                logger.info(f"Unvisited safe locations: {self.kb.get_safe_unvisited_locations()}")
            ...
            logger.info(f"Heading to location {get_final_location(from_location=self.kb.agent_location, with_orientation=self.kb.agent_orientation, with_plan_actions=self.plan_actions)[0]} ")
            logger.info(f"with plan cost {get_final_location(from_location=self.kb.agent_location, with_orientation=self.kb.agent_orientation, with_plan_actions=self.plan_actions)[1]}")
            logger.info(f"plan: {self.plan_actions}\n")

        if not self.plan_actions and self.kb.is_arrow_available and len(self.kb.possible_wumpus_locations) > 1:
            logger.info("")
            logger.warning("*" * 7 + " " * 2 + "Calculating plan to try and shoot the closest of the wumpuses that is not on a pit location")
            logger.info(f"Possible wumpus to kill: {self.kb.possible_wumpus_locations}")

            possible_wumpus_locations = self.kb.possible_wumpus_locations.intersection(self.kb.no_pit_locations)

            if possible_wumpus_locations:
                logger.info(f"Wumpus(es) with 0.0 pit probability: {possible_wumpus_locations}")
                ...
                if self.plan_actions:
                    ...
                    logger.critical("Couldn't find any plan to shoot wumpus even if possible wumpus exists")
            else:
                logger.info("No plan to shoot the wumpus, cause all wumpuses on possible pit locations")

        if not self.plan_actions:
            logger.warning("*" * 7 + " " * 2 + "Calculating plan to go and visit the safest location from the fringe locations")
            logger.info(f"Fringe locations: {self.kb.fringe_locations}")
            
            self.kb.update_pit_locations()

            logger.info(f"Found the following pit locations with SAT-Solver: {self.kb.pit_locations}")
            ...
            if max_safe_probability > MIN_SAFE_PROBABILITY_TRESHOLD:
                ...
                logger.info("No location with higher safe probability than risk treshold")

        if not self.plan_actions:
            logger.warning("*" * 7 + " " * 2 + "Calculating plan to go home without gold :(")
            self.plan_actions.extend(self.plan_route(to_goal_locations=self.kb.exit_locations,
                                                     with_allowed_locations=self.kb.get_safe_from_pit_but_possible_wumpus_locations(),
                                                     from_agent_location=self.kb.agent_location,
                                                     from_agent_orientation=self.kb.agent_orientation,
    ...
    def plan_route(self, *, to_goal_locations, with_allowed_locations, from_agent_location, from_agent_orientation, 
        ...
        logger.warning(f"{'*' * 9}  It seems you want to calculate a plan to a goal location {goal_locations} which is not included in the allowed locations {allowed_locations }" if not goal_locations.intersection(allowed_locations) else "")
